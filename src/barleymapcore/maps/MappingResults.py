...
class MappingResult(object):
    ...
    def init_from_data(mapping_data, map_name, chrom_dict, map_is_physical, map_has_cm_pos, map_has_bp_pos):
        ...
        if map_is_physical:
            ...
            if map_has_cm_pos and map_has_bp_pos:
                ...
                raise m2pException(
                    "Map configuration is wrong: has not cm nor bp positions.")

        has_multiple_pos = True if mapping_data[pos_shift] == "Yes" or mapping_data[pos_shift] == True else False
        has_other_alignments = True if mapping_data[pos_shift +
                                                    1] == "Yes" or mapping_data[pos_shift + 1] == True else False
    ...
    def __str__(self):
        return " - ".join([self._marker_id, str(self._chrom_name)+"/"+str(self._chrom_order),
                           str(self._cm_pos), str(self._cm_end_pos), str(
                               self._bp_pos), str(self._bp_end_pos), str(self._genetic), str(self._strand),
                           str(self._multiple_pos), str(
                               self._other_alignments), str(self._map_name),
