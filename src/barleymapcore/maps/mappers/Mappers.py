...
class Mapper(object):
    ...
    def create_map(self, mapping_results, mapping_unmapped, map_config, sort_param, multiple_param=False):
        raise m2pException(
            "The 'create_map' method must be called from a Mapper derived class, which implements it.")
    ...
    def _finish_map(self, sorted_positions, unaligned_markers, unmapped_markers,
        ...
        if unaligned_markers:
            unaligned_markers = sorted(
                unaligned_markers, key=lambda sorting: sorting)
        ...
        mapping_results.set_fine_mapping(
            self._single_chromosome(sorted_positions))
    ...
    def _createPositions(self, markers_positions, multiple_param, chrom_dict, map_name):
        ...
        for marker_id in markers_positions:
            ...
            if num_marker_pos == 0:
                continue  # contigs without position
            if (not multiple_param) and (num_marker_pos > 1):
                if self._verbose:
                    sys.stderr.write(
                        "Mappers: discarded multiple pos marker: "+str(marker_id)+"\n")
            ...
            for pos in positions:
                ...
                if not chr_pos in chrom_dict:
                    continue

                # Numeric value of chromsome (for sorting purposes)
                chrom_order = chrom_dict[chr_pos]

                mapping_result = MappingResult(marker_id, chr_pos, chrom_order,
                                               pos["cm_pos"], pos["cm_end_pos"], pos["bp_pos"], pos["bp_end_pos"], pos["genetic"], pos["strand"],
    ...
    def _get_unmapped_markers(self, markers_positions):
        ...
        for marker_id in markers_positions:
            ...
            if len(hits_no_pos) == 0:
                continue  # if has all the contigs with map position, continue
        ...
        positions_list = sorted(positions_list, key=lambda sorting: (
...
class DatasetMapper(Mapper):
    ...
    def create_map(self, mapping_results, mapping_unmapped, map_config, sort_param, multiple_param=False):
        # Sort the list
        sorted_positions = self._sort_positions_list(
            mapping_results, sort_param)
...
class PhysicalMapper(Mapper):
    ...
    def create_map(self, alignment_results, unaligned_markers, map_config, sort_param, multiple_param):
        ...
        positions_list = self._createPositions(
            markers_positions, multiple_param, chrom_dict, map_name)

        if self._verbose:
            sys.stderr.write("PhysicalMapper: Map: "+str(map_name)+"\n")
            sys.stderr.write("\t total positions --> " +
                             str(len(positions_list))+"\n")

        # Sort the list
        sorted_positions = self._sort_positions_list(
            positions_list, sort_param)
    ...
    def _reformatPositions(self, alignment_results):
        ...
        for alignment in alignment_results:  # [db]:
            ...
            if marker_id in markers_positions:
                ...
                marker_pos = []
                markers_positions[marker_id] = {
                    "positions": marker_pos, "hits_no_position": []}
...
class AnchoredMapper(Mapper):
    ...
    def create_map(self, alignment_results, unaligned_markers, map_config, sort_param, multiple_param):
        ...
        # This is essential to avoid this key to be used as a marker_id
        ...
        map_contigs_positions = self._mapReader.obtain_map_positions(
            contig_set)

        if self._verbose:
            sys.stderr.write("\t positions: " +
                             str(len(map_contigs_positions))+"\n")

        # Translate positions of aligned markers to map positions,
        # and detect markers which hit to contigs without map position
        markers_positions = self._resolvePositions(
            map_contigs_positions, markers_dict)
        ...
        for marker_id in markers_dict:
            ...
            for contig_tuple in markers_dict[marker_id]:
                ...
                if contig_id in map_contigs_positions:
                    ...
                    markers_positions[marker_id]["hits_no_position"].append(
                        contig_id)
