...
class MapHeaders(object):
    ...
    PHYSICAL_GENETIC = 4
    PHYSICAL_STRAND = 5
    PHYSICAL_MULTIPLE_POS = 6
    PHYSICAL_OTHER_ALIGNMENTS = 7
    PHYSICAL_MAP_NAME = 8

    OUTPUT_HEADERS = ["Marker", "chr", "cM", "base_pairs",
                      "multiple_positions", "other_alignments", "Map"]
    PHYSICAL_HEADERS = ["Marker", "chr", "start", "end", "genetic",
                        "strand", "multiple_positions", "other_alignments", "Map"]
    FEATURE_HEADERS = ["Feature", "Feature_type", "Dataset", "chr", "cM", "bp"]

    ANNOT_HEADERS = ["Description", "InterPro",
                     "Signatures", "PFAM server", "GO terms"]
...
class AlignmentsPrinter(object):
    ...
    def output_results(self, aligned, databases_ids=None):
        raise Exception(
            "To be implemented in child classes inheriting from AlignmentsPrinter")
    ...
    def print_records_db(self, aligned, db_entry, db_name):
        ...
        for alignment_result in aligned:  # db_results:
            ...
            if alignment_result.get_db_id() != db_entry:
                continue
    ...
    def print_records(self, aligned):
        ...
        for alignment_result in aligned:  # db_results:
            ...
            db_name = self._databases_config.get_database_name(
                alignment_result.get_db_id())
...
class OutputPrinter(object):
    ...
    def print_maps(self, maps_dict, show_genes, show_markers, show_anchored, show_unmapped, show_unaligned, multiple_param, load_annot, annotator):
        ...
        for map_id in maps_dict:
            ...
            if not (show_genes or show_markers or show_anchored):
                ...
                self.print_map(mapping_results.get_mapped(),
                               map_config, multiple_param)

            elif show_genes:
                # OUTPUT FOR MAP WITH GENES IF REQUESTED

                self.print_map_with_genes(mapping_results.get_map_with_genes(
                ), map_config, multiple_param, load_annot, annotator)

            elif show_markers:
                # OUTPUT FOR MAP WITH MARKERS

                self.print_map_with_markers(
                    mapping_results.get_map_with_markers(), map_config, multiple_param)

            elif show_anchored:
                # OUTPUT FOR MAP WITH ANCHORED FEATURES

                self.print_map_with_anchored(
                    mapping_results.get_map_with_anchored(), map_config, multiple_param)
            ...
            if show_unaligned:
                self.print_unaligned(
                    mapping_results.get_unaligned(), map_config)
    ...
    def print_map(self, mapping_results, map_config, multiple_param):
        ...
        sys.stderr.write(
            "OutputFacade: creating output for map "+str(map_name)+"\n")
        ...
        if self._show_headers:
            headers_row = self.output_base_header(
        ...
        for pos in positions:
            current_row = self.output_base_pos(
                pos, map_as_physical, map_has_cm_pos, map_has_bp_pos, multiple_param)

            self._output_desc.write(
                "\t".join([str(x) for x in current_row])+"\n")

        sys.stderr.write("\tgenetic maps printed.\n")

        if self._verbose:
            sys.stderr.write("\tlines printed "+str(len(positions))+"\n")
    ...
    def print_map_with_genes(self, mapping_results, map_config, multiple_param, load_annot, annotator):
        ...
        sys.stderr.write(
            "OutputFacade: creating output for map with genes "+str(map_name)+"\n")
    ...
    def print_map_with_markers(self, mapping_results, map_config, multiple_param):
        ...
        sys.stderr.write(
            "OutputFacade: creating output for map with markers "+str(map_name)+"\n")
        ...
        if self._show_headers:
            ...
            headers_row = self.output_features_header(
                map_as_physical, map_has_cm_pos, map_has_bp_pos, multiple_param)
    ...
    def print_map_with_anchored(self, mapping_results, map_config, multiple_param):
        ...
        sys.stderr.write(
            "OutputFacade: creating output for map with anchored features "+str(map_name)+"\n")
    ...
    def print_unmapped(self, mapping_results, map_config):
        ...
        sys.stderr.write(
            "OutputFacade: creating output for unmapped of "+str(map_name)+" map\n")
    ...
    def _output_unmapped(self, unmapped_records):
        ...
        if self._verbose:
            sys.stderr.write("\tUnmapped records: " +
                             str(len(unmapped_records))+"\n")
    ...
    def print_unaligned(self, mapping_results, map_config):
        ...
        sys.stderr.write(
            "OutputFacade: creating output for unaligned of "+str(map_name)+" map\n")
    ...
    def _output_unaligned(self, unaligned_records):
        ...
        if self._verbose:
            sys.stderr.write("Unaligned records: " +
                             str(len(unaligned_records))+"\n")
    ...
    def output_base_header(self, map_as_physical, map_has_cm_pos, map_has_bp_pos, multiple_param):
        ...
        if map_as_physical:
            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_ID])
            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_CHR])
            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_START_POS])
            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_END_POS])
            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_GENETIC])
            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_STRAND])

            if multiple_param:
                current_row.append(
                    MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_MULTIPLE_POS])

            current_row.append(
                MapHeaders.PHYSICAL_HEADERS[MapHeaders.PHYSICAL_OTHER_ALIGNMENTS])

        else:
            current_row.append(
                MapHeaders.OUTPUT_HEADERS[MapHeaders.MARKER_NAME_POS])
            current_row.append(
                MapHeaders.OUTPUT_HEADERS[MapHeaders.MARKER_CHR_POS])
            if map_has_cm_pos:
                current_row.append(
                    MapHeaders.OUTPUT_HEADERS[MapHeaders.MARKER_CM_POS])

            if map_has_bp_pos:
                current_row.append(
                    MapHeaders.OUTPUT_HEADERS[MapHeaders.MARKER_BP_POS])

            if multiple_param:
                current_row.append(
                    MapHeaders.OUTPUT_HEADERS[MapHeaders.MULTIPLE_POS])

            current_row.append(
                MapHeaders.OUTPUT_HEADERS[MapHeaders.OTHER_ALIGNMENTS])
    ...
    def output_base_pos(self, pos, map_as_physical, map_has_cm_pos, map_has_bp_pos, multiple_param):
        ...
        # Marker ID
        # [MapFields.MARKER_NAME_POS]))
        current_row.append(str(pos.get_marker_id()))
        ...
        if map_as_physical:
            ...
            current_row.append(pos.get_genetic())
        ...
        if pos.is_empty():
            ...
            if multiple_param:
                ...
                if mult:
                    current_row.append("Yes")
                else:
                    current_row.append("No")
...
class ExpandedPrinter(OutputPrinter):
    ...
    def output_features_header(self, map_as_physical, map_has_cm_pos, map_has_bp_pos, multiple_param, load_annot=False, annotator=None):
        ...
        base_headers_row = self.output_base_header(
            map_as_physical, map_has_cm_pos, map_has_bp_pos, multiple_param)
        headers_row.extend(base_headers_row)

        headers_row.append(
            MapHeaders.FEATURE_HEADERS[GenesFields.GENES_ID_POS])
        # headers_row.append(MapHeaders.FEATURE_HEADERS[GenesFields.GENES_TYPE_POS])
        headers_row.append(
            MapHeaders.FEATURE_HEADERS[GenesFields.GENES_DATASET_NAME_POS])
        headers_row.append(
            MapHeaders.FEATURE_HEADERS[GenesFields.GENES_CHR_POS])
        ...
        if map_as_physical:
            ...
            if map_has_cm_pos:
                headers_row.append(
                    MapHeaders.FEATURE_HEADERS[GenesFields.GENES_CM_POS])

            if map_has_bp_pos:
                headers_row.append(
                    MapHeaders.FEATURE_HEADERS[GenesFields.GENES_BP_POS])
